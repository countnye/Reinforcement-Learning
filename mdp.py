...
class Map:
    ...
    def valueIteration(self):
        ## initialize utilities to 0 (1)
        for state in self.states.values():
            if not state.isGoal:
                state.utility = 0.0
        ## begin value iteration
        util_change = [0 for _ in range(len(self.states))]
        while True:
            change = 0
            for state in self.states.values():
                if not state.isGoal:
                    prev_value = state.utility
                    sum_arr = []
                    for action in state.actions:
                        sum = 0
                        for possible_action in state.transitions[action]:
                            sum += possible_action[0] * possible_action[1].utility
                        sum_arr.append(sum)
                    state.utility = state.reward + (self.gamma * max(sum_arr))
                    change = abs(state.utility - prev_value)
                    util_change[state.id] = change
            if max(util_change) < self.stop_crit:
                break

    def policyIteration(self):
        ## For each state, that isn't a goal, init the state's policy 
        ## by choosing a random action. Also set the state's previous
        ## policy.
        for state in self.states.values():
            if not state.isGoal:
        ...
        ## Stop if all states haven't changed
        stop = False
        while not stop:
            num_unstable_states = 0
            ## Calculate utility for all states
            self.calculateUtilitiesLinear()
            ## Update non-goal states policy by selecting
            ## best action.
            for state in self.states.values():
                if not state.isGoal:
                    state.policy = state.selectBestAction()
                    if state.policy != state.previous_policy:
                        num_unstable_states += 1

                    ## Update previous policy
                    state.previous_policy = state.policy

            ## Break out of the loop if none of the states
            ## have changed
            if num_unstable_states == 0:
                stop = True
